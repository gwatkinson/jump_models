# Taken from https://github.com/datamol-io/graphium/blob/main/graphium/nn/architectures/global_architectures.py
...
class FeedForwardNN(nn.Module, MupMixin):
    def __init__(
        ...
        r"""A flexible neural network architecture, with variable hidden
        dimensions, support for multiple layer types, and support for different
        ...
        self.residual_class, self.residual_name = self._parse_class_from_dict(
            residual_type, RESIDUALS_DICT
        )
    ...
    def _check_bad_arguments(self):
        r"""Raise comprehensive errors if the arguments seem wrong."""
    ...
    def _parse_class_from_dict(
        ...
        r"""Register the hyperparameters for tracking by Pytorch-lightning."""
        if isinstance(name_or_class, str):
            ...
            raise TypeError(
                f"`name_or_class` must be str or callable, provided: {type(name_or_class)}"
            )

        return obj_class, obj_name

    def _create_residual_connection(
        self, out_dims: List[int]
    ) -> Tuple[ResidualConnectionBase, List[int]]:
        r"""Create the residual connection classes.

        The out_dims is only used if the residual classes requires
        weights
        """
    ...
    def _create_layers(self):
        r"""Create all the necessary layers for the network. It's a bit
        complicated to explain what's going on in this function, but it must
        manage the varying features sizes caused by:

        - The presence of different types of residual connections
        """

        self.residual_layer, residual_out_dims = self._create_residual_connection(
    ...
    def get_init_kwargs(self) -> Dict[str, Any]:
        """Get a dictionary that can be used to instanciate a new object with
    ...
    def make_mup_base_kwargs(
        self, divide_factor: float = 2.0, factor_in_dim: bool = False
    ) -> Dict[str, Any]:
        """Create a 'base' model to be used by the `mup` or `muTransfer`
        scaling of the model. The base model is usually identical to the
        regular model, but with the layers width divided by a given factor (2
        by default)
    ...
    def __repr__(self):
        r"""Controls how the class is printed."""
...
class FeedForwardGraph(FeedForwardNN):
    def __init__(
        ...
        if len(self.hidden_dims_edges) > 0:
            self.full_dims_edges = (
                [self.in_dim_edges] + self.hidden_dims_edges + [self.hidden_dims_edges[-1]]
            )
    ...
    def _create_layers(self):
        ...
        residual_layer_temp, residual_out_dims = self._create_residual_connection(
            out_dims=self.full_dims[1:]
        )
        ...
        if self.full_dims_edges is not None:
            ...
            residual_out_dims_edges = residual_layer_temp.get_true_out_dims(
                self.full_dims_edges[1:]
            )
        ...
        for ii in range(self.depth):
            ...
            if self.layer_class.layer_supports_edges and self.in_dim_edges > 0:
                ...
                if (
                    "out_dim_edges"
                    in inspect.signature(self.layer_class.__init__).parameters.keys()
                ):
                    if self.full_dims_edges is not None:
            ...
            if ii < len(residual_out_dims):
                ...
                if self.full_dims_edges is not None:
                    this_in_dim_edges = (
                        residual_out_dims_edges[ii] * self.layers[ii - 1].out_dim_factor
                    )
        ...
        if len(layer_out_dims_edges) > 0:
            self.residual_edges_layer, _ = self._create_residual_connection(
                out_dims=layer_out_dims_edges
            )
        else:
    ...
    def forward(self, g: Batch) -> torch.Tensor:
        r"""Apply the full graph neural network on the input graph and node
        features.
    ...
    def make_mup_base_kwargs(
        ...
        kwargs = self.get_init_kwargs()
        kwargs["hidden_dims"] = [round(dim / divide_factor) for dim in kwargs["hidden_dims"]]
        kwargs["hidden_dims_edges"] = [
            round(dim / divide_factor) for dim in kwargs["hidden_dims_edges"]
        ]
        if factor_in_dim:
            kwargs["in_dim"] = round(kwargs["in_dim"] / divide_factor)
...
class FullGraphMultiTaskNetwork(nn.Module, MupMixin):
    def __init__(
        ...
        r"""Class that allows to implement a full graph neural network
        architecture, including the pre-processing MLP and the post processing
        MLP.
    ...
    def _check_bad_arguments(self):
        ...
        if self.task_heads is not None:
            ...
            if len(graph_level_tasks) > 0 and self.graph_output_nn_kwargs["graph"]["pooling"] == [
                "none"
            ]:
                raise ValueError(
    ...
    def _apply_ipu_pipeline_split(self, gnn_layers_per_ipu):
        r"""Apply pipeline split from accelerator options if applicable."""
        ...
        for begin_block_layer_index, ipu_id in zip(
            begin_block_layer_indices, range(1, pipeline_length)
        ):
    ...
    def forward(self, g: Batch) -> Tensor:
        r"""Apply the pre-processing neural network, the graph neural network,
    ...
    def make_mup_base_kwargs(self, divide_factor: float = 2.0) -> Dict[str, Any]:
        ...
        if self.pre_nn is not None:
            ...
            pe_enc_outdim = (
                0 if self.encoder_manager is None else self.pe_encoders_kwargs.get("out_dim", 0)
            )
            pre_nn_indim = kwargs["pre_nn_kwargs"]["in_dim"] - pe_enc_outdim
            kwargs["pre_nn_kwargs"]["in_dim"] = round(
                pre_nn_indim + (pe_enc_outdim / divide_factor)
            )
        ...
        if self.pre_nn_edges is not None:
            ...
            pe_enc_edge_outdim = (
                ...
                else self.pe_encoders_kwargs.get("edge_out_dim", 0)
            )
    ...
    def set_max_num_nodes_edges_per_graph(
        self, max_nodes: Optional[int], max_edges: Optional[int]
    ) -> None:
        """Set the maximum number of nodes and edges for all gnn layers and
        encoder layers.
    ...
    @property
    def in_dim(self) -> int:
        r"""Returns the input dimension of the network."""
        if self.pre_nn is not None:
    ...
    def out_dim(self) -> int:
        r"""Returns the output dimension of the network."""
        return self.gnn.out_dim

    @property
    def out_dim_edges(self) -> int:
        r"""Returns the output dimension of the edges of the network."""
    ...
    def in_dim_edges(self) -> int:
        r"""Returns the input edge dimension of the network."""
        return self.gnn.in_dim_edges
...
class GraphOutputNN(nn.Module, MupMixin):
    def __init__(
        ...
        filtered_graph_output_nn_kwargs = {
            k: v
            for k, v in graph_output_nn_kwargs[self.task_level].items()
            if k not in ["pooling", "in_dim"]
        }
    ...
    def _pool_layer_forward(
        ...
        r"""Apply the graph pooling layer, followed by the linear output layer.
    ...
    def drop_graph_output_nn_layers(self, num_layers_to_drop: int) -> None:
        r"""Remove the last layers of the model.
    ...
    def extend_graph_output_nn_layers(self, layers: nn.ModuleList):
        r"""Add layers at the end of the model.
    ...
    def concat_last_layers(self) -> Optional[Iterable[int]]:
        """Property to control the output of the `self.forward`. If set to a
        list of integer, the `forward` function will concatenate the output of
        different layers.
    ...
    def concat_last_layers(self, value: Union[Type[None], int, Iterable[int]]) -> None:
        """Set the property to control the output of the `self.forward`. If set
        to a list of integer, the `forward` function will concatenate the
        output of different layers. If a single integer is provided, it will
        output that specific layer.
...
class TaskHeads(nn.Module, MupMixin):
    def __init__(
        ...
        self.task_levels = {
            head_kwargs["task_level"] for _, head_kwargs in self.task_heads_kwargs.items()
        }
        self.in_dim = in_dim
        self.in_dim_edges = in_dim_edges
    ...
    def forward(self, g: Batch) -> Dict[str, torch.Tensor]:
        ...
        """
        features = {
            task_level: self.graph_output_nn[task_level](g) for task_level in self.task_levels
        }
    ...
    def make_mup_base_kwargs(
        ...
        for task_name, task_nn in self.task_heads.items():
            ...
            task_heads_kwargs[task_name]["task_level"] = self.task_heads_kwargs[task_name][
                "task_level"
            ]
        kwargs = dict(
            in_dim=self.in_dim,
    ...
    def out_dim(self) -> Dict[str, int]:
        r"""Returns the output dimension of each task head."""
        return {task_name: head.out_dim for task_name, head in self.task_heads.items()}

    def __repr__(self):
        r"""Returns a string representation of the task heads."""
        task_repr = []
