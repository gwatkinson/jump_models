# Taken from https://github.com/facebookresearch/multimodal/blob/main/torchmultimodal/modules/fusions/
...
class AttentionFusionModule(nn.Module):
    """Fuse embeddings through weighted sum of the corresponding linear
    projections. Linear layer for learning the weights.
    ...
    def __init__(
        ...
        if self.use_auto_mapping:
            self.projections = nn.ModuleDict(
                {channel: nn.Linear(dim, projection_dim) for channel, dim in channel_to_encoder_dim.items()}
            )
        else:
...
def deepset_transformer(
    ...
    """
    projection_dim = DeepsetFusionWithTransformer.get_projection_dim(channel_to_encoder_dim, use_auto_mapping)
