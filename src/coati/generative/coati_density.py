...
def estimate_density_batchwise(
    ...
    """Simple batchwise density estimation of multivariate normal distribution
    of embedding space given SMILES strings."""
    mean_param = torch.nn.Parameter(torch.zeros(encoder.embed_dim, device=encoder.device))
    sqrt_diag_param = torch.nn.Parameter(
        0.5 * torch.ones(encoder.embed_dim, device=encoder.device)
    )  # has to be positive.
    lower_diag_indices = torch.tril_indices(encoder.embed_dim, encoder.embed_dim, offset=-1)
    lower_tri_param = torch.nn.Parameter(torch.zeros(lower_diag_indices.shape[-1], device=encoder.device))

    def build_distribution(sq_diag: torch.Tensor, lower_tri: torch.Tensor) -> MultivariateNormal:
            ...
            for S in batch:
                try:
                    ...
                    batch_tokens_.append(
                        tokenizer.tokenize_text("[SMILES]" + canonical_smiles[-1] + "[STOP]", pad=True)
                    )
                except:
                    continue
            with torch.no_grad():
                batch_tokens = torch.tensor(batch_tokens_, device=encoder.device, dtype=torch.int)
