...
def embed_altair(
    ...
    """
    df["image"] = df[smiles_field].apply(wrapped_get_smiles_image).apply(image_formatter2).copy()
    df["mol_index"] = range(df.shape[0])

    if not selector_field is None:
        substrate_frame = pd.DataFrame({selector_field: df[selector_field].unique().tolist()})
        selection = alt.selection_multi(fields=[selector_field])
        color1 = alt.condition(selection, alt.Color(selector_field + ":N"), alt.value("lightgray"))
        substrate_selector = (
            alt.Chart(substrate_frame).mark_rect().encode(y=selector_field, color=color1).add_selection(selection)
        )
