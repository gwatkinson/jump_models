...
def embed_smiles(s: str, encoder: e3gnn_smiles_clip_e2e, tokenizer: TrieTokenizer) -> torch.Tensor:
...
def purify_vector(V: torch.Tensor, encoder: e3gnn_smiles_clip_e2e, tokenizer: TrieTokenizer, n_rep=128) -> torch.Tensor:
    """purification is usually the name given to an operation which pulls out
    the idempotent part of a vector under a map.
    ...
    with torch.no_grad():
        try:
            regen_smiles = encoder.hclip_to_2d_batch(V.to(encoder.device).unsqueeze(0).repeat(n_rep, 1), tokenizer)
        except Exception as Ex:
            return V
        batch_tokens_ = []
        for S in regen_smiles:
            try:
                S = Chem.MolToSmiles(Chem.MolFromSmiles(S))
                batch_tokens_.append(tokenizer.tokenize_text("[SMILES]" + S + "[STOP]", pad=True))
            except Exception as Ex:
                pass
        if len(batch_tokens_) < 1:
            return V
        batch_tokens = torch.tensor(batch_tokens_, device=encoder.device, dtype=torch.long)
        batch_embeds = encoder.encode_tokens(batch_tokens, tokenizer)
...
def force_decode_valid_batch(
    ...
    """Attemps multiple parallel decodings until a valid SMILES string is
    produced.

    If multiple valid SMILES strings are produced, returns the most
    common one.
    """
    for k in range(max_attempts):
        try:
            with torch.no_grad():
                regen_smiles = encoder.hclip_to_2d_batch(V.unsqueeze(0).repeat(batch_size, 1), tokenizer)
                slist = []
                for S in regen_smiles:
                    try:
