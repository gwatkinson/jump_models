...
def roc_plot(source, y_true="y", y_pred="y_pred", partition_col=None, chart_args=None):
    ...
    if not (partition_col is None):
        ...
        train_df = pd.DataFrame({"False Positive Rate": train_fpr, "True Positive Rate": train_tpr})
        ...
        test_df = pd.DataFrame({"False Positive Rate": test_fpr, "True Positive Rate": test_tpr})
        test_df[partition_col] = f"test - auROC: {test_auc}"

        return (
            alt.Chart(pd.concat([train_df, test_df]), **chart_args)
            .mark_line()
            .encode(x="False Positive Rate", y="True Positive Rate", color=partition_col)
        )
    else:
        test_fpr, test_tpr, _ = roc_curve(
