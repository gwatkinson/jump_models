...
def makedir(path: str, isfile: bool = False):
    """Creates a directory given a path to either a directory or file. If a
    directory is provided, creates that directory. If a file is provided (i.e.
    isfile == True), creates the parent directory for that file.
...
class OnlineEstimator:
    """Simple storage-less Knuth estimator which accumulates mean and
    variance."""
...
def records_mp(recs, func, args=None, n=None):
    """Apply func(chunk_recs, *args) to chunks of input records using
    multiprocessing."""
    if n is None:
