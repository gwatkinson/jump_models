...
from rdkit.Chem import Crippen, Descriptors, Draw, Lipinski, PandasTools, rdMolDescriptors
from rdkit.Chem.AllChem import EmbedMolecule, EmbedMultipleConfs, GetMorganFingerprintAsBitVect
...
def works_on_smiles(raise_on_failure: bool):
    """converts any function mapping a mol to another mol (or any output) to a
    function that also works with smiles.
            ...
            if isinstance(args[0], str):
                if mol := Chem.MolFromSmiles(args[0]):
                    ...
                    if isinstance(results, Chem.Mol):
                        ...
                        return tuple(Chem.MolToSmiles(res) if isinstance(res, Chem.Mol) else res for res in results)
                    else:
                        return results
                else:
                    if raise_on_failure:
...
def sim_mol(mol1: Chem.Mol, mol2: Chem.Mol) -> float:
    """Simple wrapper over ECFP4 tanimoto similarity.
...
def identical_canonsmi(smi1: str, smi2: str, use_chiral: int = 1) -> bool:
    return Chem.CanonSmiles(smi1, useChiral=use_chiral) == Chem.CanonSmiles(smi2, useChiral=use_chiral)
...
def draw_smi_grid(smis: List[str], mols_per_row=5, sub_img_size=(300, 300), legends=None):
...
def mol_to_atoms_coords(
    ...
    if optimize and hydrogenate:
        try:
            ...
            opt = MMFFOptimizeMoleculeConfs(m3, mmffVariant="MMFF94s", numThreads=numThreads, maxIters=10000)
            opt = np.array(opt)
            converged = opt[:, 0] == 0
