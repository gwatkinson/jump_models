...
class RotaryEmbedding(torch.nn.Module):
    def __init__(
        ...
        """Eq. (34) of https://arxiv.org/pdf/2104.09864.pdf also inspired by
        https://blog.eleuther.ai/rotary-embeddings/ The rotation is done after
        the hidden dimension is split into heads. so, the cached sin/cos
        tensors operate on a space (n_embd // n_head)
        ...
        inv_freq = 1.0 / (base ** (torch.arange(0, (n_embd // n_head), 2, device=device).float() / (n_embd // n_head)))
...
class RotarySelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.register_buffer(
            "bias",
            torch.tril(torch.ones(config.n_seq, config.n_seq)).view(1, 1, config.n_seq, config.n_seq),
        )
...
class CausalSelfAttention(nn.Module):
    ...
    It is possible to use torch.nn.MultiheadAttention here but I am
    including an explicit implementation here to show that there is
