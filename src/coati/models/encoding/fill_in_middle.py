...
def adj_mat_to_tokens(
    ...
    """This is basically the TokenGT paper the reverse is
    decode_tokenized_graph()"""
    ...
    try:
        ...
        light_to_heavy[adj_mat_atoms > 1] = np.arange((adj_mat_atoms > 1).sum(), dtype=int)

        for I, at in enumerate([adj_mat_atoms[K] for K in range(len(adj_mat_atoms))]):
            if only_heavy:
                if at < 2:
                    continue
            atom_string = atom_string + "[NUM" + str(light_to_heavy[I]) + "]" + "[ELM" + str(int(at)) + "]"
        for I, ed in enumerate(adj_mat):
            ...
            edge_numbers = sorted([light_to_heavy[int(ed[0])], light_to_heavy[int(ed[1])]])
            edge_string = edge_string + et + "[NUM" + str(edge_numbers[0]) + "]" + "[NUM" + str(edge_numbers[1]) + "]"
