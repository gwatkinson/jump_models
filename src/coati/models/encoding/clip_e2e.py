...
def clip_ar_xform(
    ...
    """This version randomly augments data in several ways (front or back) it
...
def _tokenize_smiles(smi, tokenizer, prefix="[SMILES]", suffix="[STOP]", device="cpu", max_size=None):
    if max_size is None:
...
class e3gnn_smiles_clip_e2e(nn.Module):
    ...
    def points_to_2d(
        ...
        """Generation of SMILES (or GRAPH) from atoms and coords."""
    ...
    def hclip_to_2d_batch(
        ...
        """Decodes a batch of h_clip embeddings into a batch of SMILES (or
        GRAPH)"""
        if noise_scale > 0:
            h_clip += torch.normal(mean=torch.zeros_like(h_clip), std=noise_scale * torch.ones_like(h_clip))
        h_token = self.point_clip_to_special_tokens(h_clip)
    ...
    def batch_smiles_to_s2s_likelihood(self, smiles: List[str], tokenizer: TrieTokenizer) -> torch.Tensor:
