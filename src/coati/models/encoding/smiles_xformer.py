...
def get_stop_token_embs(x: torch.Tensor, idx: torch.Tensor, tokenizer):
    ...
    if not stop_embs.shape[0] == x.shape[0]:
        raise RuntimeError("Some smiles in the batch do not have stop tokens. Did some tokenizations fail?")
...
class RotarySmilesTransformer(nn.Module):
    ...
    def encode(self, idx, tokenizer):
        """Only returns the vector of the [STOP] token which MUST be the last
        token before [PAD]"""
        x = self.xformer(idx)
    ...
    def generate_greedy(self, prefix=torch.tensor([[1]]), stop_token=2, max_len=256):
        ...
        c.f. https://huggingface.co/blog/how-to-generate
        Note: the decode from forward is not autoregressive.
        this stops upon hitting a stop token.
        """
        generated = torch.clone(prefix)
        with torch.no_grad():
            while (generated.flatten()[-1].item() != stop_token) and generated.shape[1] < self.n_seq:
    ...
    def generate_topk(
        ...
        with torch.no_grad():
            while (generated.flatten()[-1].item() != stop_token) and generated.shape[1] < self.n_seq:
                ...
                generated = torch.cat([generated, (inds[inds_of_inds]).unsqueeze(0).unsqueeze(0)], 1)
    ...
    def generate_topk_batch(self, prefix=[[0]], stop_token=2, pad_token=0, inv_temp=2, k=10):
        ...
        for K, row in enumerate(prefix):
            prefix_t[K, : len(row)] = torch.tensor(row, device=self.lm_head.weight.device, dtype=torch.long)
    ...
    def xformer_blocks(self, x: torch.Tensor, apply_norm: bool = True, output_logits: bool = False) -> torch.Tensor:
        for block in self.transformer.h:
            x = block(x, self.emb)
        if apply_norm:
            x = self.transformer.ln_f(x)
    ...
    def generate_topk_with_inj(
        ...
        """Like the above, but works in the embedding space rather than token
        space, so it can do clip injection.
        ...
        https://arxiv.org/pdf/1805.04833.pdf
        """
        assert len(prefix) <= self.n_seq, f"Cannot forward sequence of length {len(prefix)}, n_seq is only {self.n_seq}"
        prefix_x = self.emb(torch.tensor(prefix, device=inj_payload.device, dtype=torch.long).unsqueeze(0))
        # Inject the payload
        ...
        with torch.no_grad():
            while (last_token != stop_token) and len(generated) < self.n_seq - 1:
                if len(generated):
                    ...
                    gen_x = self.emb(torch.tensor(generated, device=inj_payload.device, dtype=torch.long).unsqueeze(0))
    ...
    def generate_top_k_with_inj_batch(
        ...
        if as_tensor:
            return torch.cat(
                [
                    torch.tensor(prefix, dtype=torch.long, device=generated.device).unsqueeze(0).repeat(batch_size, 1),
                    generated,
                ],
                dim=1,
            )
    ...
    def forward_with_stop_emb(self, idx, tokenizer):
        """I made this a separate routine because of issues with torch
        DataParallel and functions with variable numbers of return values.
    ...
    def forward_with_stop_emb_and_replacement(self, idx, injection, tokenizer, inject_token="[UNK]"):
        """This is specifically for e2e-CLIP a-la clipCAP. It injects tokens in
        place of special_token and also returns the stop-emb.
    ...
    def forward_with_replacement(self, idx, injection, tokenizer, inject_token="[UNK]"):
        """This is specifically for e2e-CLIP a-la clipCAP. It injects tokens in
        place of special_token SORRY FOR THE REPETITION. YOU KNOW ABOUT DUMB
        VARIABLE RETURN NUMBER ISSUES IN TORCH JIT  JAP-3-29-2023.
