...
def clip_ar_xform(
    ...
    """This verison randomly augments data in several ways (front or back) it
    randomly prepends the dataset, it randomly prepends the molecular formula
    and randomly performs the prefix, suffix, middle.
    ...
    for k, S__ in enumerate(batch["smiles"]):
        ...
        try:
            ...
            if random.random() < p_dataset:
                if "[" + batch["source_collection"][k] + "]" in tokenizer.special_tokens:
                    reps.append("set")
            if random.random() < p_formula:
                reps.append("formula")
            if random.random() < p_graph and "adj_mat" in batch and "adj_mat_atoms" in batch:
                reps.append("graph")
            ...
            for rep in reps:
                if rep == "set":
                    ...
                    if (cts < 150).all():
                        rows = np.stack([np.arange(0, cts.shape[0])[cts > 0], cts[cts > 0]], -1)
                        formula_string = "[FORMULA]" + "".join(
                            ["[ELM" + str(r[0]) + "][NUM" + str(r[1]) + "]" for r in rows]
                        )
                    else:
                        formula_string = ""
                    S = S + formula_string
                elif rep == "graph":
                    graph_string = adj_mat_to_tokens(batch["adj_mat"][k], batch["adj_mat_atoms"][k])
                    S = S + graph_string
            ...
            if random.random() < p_clip and len(ttext) > 3:
                if random.random() < p_clip_cut:
                    ...
                    while mp == sp:  #  or mp > ttext_ind_smiles:
                        ...
                        ttext = (
                            tokenizer.tokenize_text("[CLIP][UNK]", pad=False, range_check=False)
                            + ttext[:mp]
                    ...
                    ttext = tokenizer.tokenize_text("[CLIP][UNK]", pad=False, range_check=False) + ttext
                ...
                mp, sp = 1, 1
                while mp == sp:
                    mp, sp = sorted([random.randint(1, len(ttext)), random.randint(1, len(ttext))])
                ttext = (
            ...
            if random.random() < p_randsmiles:
                ...
                unnperm_toks = tokenizer.tokenize_text("[SMILES]" + S_ + "[STOP]", pad=False, range_check=False)
            else:
            ...
            if len(ttext) <= tokenizer.n_seq and len(s2s_text) <= tokenizer.n_seq:
                ...
                if len(s2s_text) <= tokenizer.n_seq and len(unnperm_toks) <= tokenizer.n_seq:
                    t = torch.zeros(tokenizer.n_seq, dtype=torch.long, device=device)
                    ...
                    s2s_stack.append(
                        torch.cat(
                            [
                                tokenizer.stop_token * torch.ones(1, dtype=torch.long, device=device),
                                torch.zeros(tokenizer.n_seq - 1, dtype=torch.long, device=device),
                            ],
                            0,
                        )
                    )
                    token_stack.append(torch.zeros(tokenizer.n_seq, dtype=torch.long, device=device))
    ...
    for col in ["tokens", "atoms", "raw_tokens"]:
        if col in batch:
            if type(batch[col]) != torch.Tensor:
                batch[col] = torch.tensor(batch[col], requires_grad=False).to(device, torch.long)
    for col in ["coords"]:
        if col in batch:
            if type(batch[col]) != torch.Tensor:
                batch[col] = torch.tensor(batch[col], requires_grad=False).to(device, dtype)
    for col in fp_targets:
        if col in batch:
            if type(batch[col]) != torch.Tensor:
                batch[col] = torch.tensor(np.stack(batch[col]), requires_grad=False).to(device, dtype)
    if coord_noise:
        batch["coords"] += torch.normal(torch.zeros_like(batch["coords"]), 0.05 * torch.ones_like(batch["coords"]))

    # decrease the sequence size to max demanded by this batch.
    batch["tokens"] = batch["tokens"][:, : (batch["tokens"].sum(0) > 0).sum()]
    batch["raw_tokens"] = batch["raw_tokens"][:, : (batch["raw_tokens"].sum(0) > 0).sum()]
...
def _tokenize_smiles(smi, tokenizer, prefix="[SMILES]", suffix="[STOP]", device="cpu"):
    try:
        ttext = tokenizer.tokenize_text(prefix + smi + suffix, pad=False, range_check=False)
...
class e3gnn_smiles_clip_e2e(nn.Module):
    ...
    and a forward pass which only requires one pass through each encoder
    type.
    """
    ...
    def __init__(
        ...
        if token_mlp:
            ...
            self.point_clip_to_special_tokens = nn.Sequential(nn.SiLU(), nn.Linear(self.embed_dim, self.embed_dim))
        else:
            self.point_clip_to_special_tokens = nn.Identity()

        self.fp_networks = torch.nn.ModuleDict({k: nn.Linear(self.embed_dim, v) for k, v in self.fp_map.items()})
    ...
    def encode_points(self, atoms, coords):
        """node_mask is made in the e3gnn enc.
    ...
    def points_to_2d(
        ...
        """Testing generation of SMILES (or GRAPH) from atoms and coords."""
        assert fill_in_from == "[SMILES]" or fill_in_from == "[GRAPH]"
        h_clip = self.encode_points(atoms, coords)
        if noise_scale > 0:
            h_clip += torch.normal(mean=torch.zeros_like(h_clip), std=noise_scale * torch.ones_like(h_clip))
        h_token = self.point_clip_to_special_tokens(h_clip)
        # create a 'batch' to infer smiles.
        token_prebatch = tokenizer.tokenize_text("[CLIP][UNK]" + fill_in_from + "[SUFFIX][MIDDLE]", pad=False)
    ...
    def hclip_to_2d_batch(
        ...
        smiles_list = [tokenizer.decode(token_out, special=keep_special) for token_out in generation]
        return smiles_list
    ...
    def hclip_and_tokens_to_likelihood(self, hclip, smiles, tokenizer):
        """Simply computes the likelihood that hclip decodes to a given
        smiles."""
        tokens = torch.tensor(
            tokenizer.tokenize_text("[CLIP][UNK][SMILES][SUFFIX][MIDDLE]" + smiles + "[STOP]", pad=False),
            device=hclip.device,
            dtype=torch.long,
        ).unsqueeze(0)
    ...
    def batch_smiles_to_s2s_likelihood(self, smiles, tokenizer):
        """Simply computes the likelihood that SMILES->hclip->SMILES decodes
        for all SMILES in a list of `smiles`"""
        # make tokens from '<smi>[STOP]'
        _tokens = [_tokenize_smiles(smi, tokenizer, prefix="", suffix="[STOP]", device=self.device) for smi in smiles]
    ...
    def smiles_to_graph(self, smiles, tokenizer, inv_temp=2, k=100):
        """Testing generation of SMILES from atoms and coords."""
        # create a 'batch' to infer smiles.
        token_prebatch = tokenizer.tokenize_text("[PREFIX][SMILES]" + smiles + "[GRAPH][SUFFIX][MIDDLE]", pad=False)
    ...
    def prefix_generate_batch(self, prefixes, tokenizer, inv_temp=2, k=100, keep_special=False, de_fim=True):
        """Testing generation of SMILES from atoms and coords."""
        # create a 'batch' to infer smiles.
        tokens = [tokenizer.tokenize_text("[PREFIX]" + p + "[SUFFIX][MIDDLE]", pad=False) for p in prefixes]
        ...
        smiles_list = [tokenizer.decode(token_out, special=keep_special, de_fim=de_fim) for token_out in generation]
        return smiles_list
    ...
    def smiles_to_graph_batch(self, smiles, tokenizer, inv_temp=2, k=100):
        ...
        # create a 'batch' to infer smiles.
        tokens = [
            tokenizer.tokenize_text("[PREFIX][SMILES]" + S + "[GRAPH][SUFFIX][MIDDLE]", pad=False) for S in smiles
        ]
        generation = self.xformer.generate_topk_batch(
            prefix=tokens,
            stop_token=tokenizer.stop_token,
        ...
        smiles_list = [tokenizer.decode(token_out, special=True) for token_out in generation]
        return smiles_list

    def forward_dist(self, raw_tokens, augmented_tokens, atoms, coords, tokenizer, p_clip_emb_smi=0.4):
        """Same as the below routine but for DistributedDataParallel
        training."""
        with autocast(enabled=False, device_type="cuda"):
            ...
            fp_preds = {fp_name: self.fp_networks[fp_name](smiles_clip_token) for fp_name in self.fp_networks}

        logits = self.xformer.forward_with_replacement(augmented_tokens, clip_token, tokenizer)
        bad_rows = augmented_tokens.sum(-1) < 1
        return h_e3gnn, h_smiles, logits, bad_rows, fp_preds

    def forward(self, raw_tokens, augmented_tokens, atoms, coords, tokenizer, p_clip_emb_smi=0.4):
        """Same as the below routine but for DistributedDataParallel
        training."""
        with autocast(enabled=False, device_type="cuda"):
            h_e3gnn = self.encode_points(atoms, coords)
