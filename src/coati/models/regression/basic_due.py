...
def basic_due(
    ...
    if type(y_field) == str:
        ...
        y = np.stack([np.stack([r[y_fieldi] for y_fieldi in y_field], -1) for r in dataset], 0)
    ...
    kernel = "RBF"
    initial_inducing_points, initial_lengthscale = initial_values(train_dataset, feature_extractor, n_inducing_points)
    ...
    if remove_spectral_norm:
        model.feature_extractor.first = torch.nn.utils.remove_spectral_norm(model.feature_extractor.first)
